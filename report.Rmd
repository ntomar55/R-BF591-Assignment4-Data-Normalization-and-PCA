...
metadata <- meta_info_from_labels(colnames(filt_v_counts[-1]))
...
lib_s <- get_library_size(filt_v_counts)
#Make a barplot showing the number of millions of reads for each sample
lib_size_tibl <- as.tibble(lib_s)
lib_size_tibl$value <- lib_size_tibl$value/10^6
lib_size_tibl$Samples <- names(filt_v_counts[-1])
plot <- ggplot(lib_size_tibl, aes(x=Samples,y=value,fill=Samples)) +
...
cpm_tbl <- normalize_by_cpm(filt_v_counts)
knitr::kable(head(cpm_tbl))
```
...
norm_deseq <- deseq_normalize(filt_v_counts, metadata)

knitr::kable(head(norm_deseq))
```
...
sample_names <- filt_v_counts[-1]
plot_sample_distributions(sample_names, TRUE, "Sample Distributions (Log10(Raw Counts))")
```
...
plot_variance_vs_mean(sample_names, TRUE, "Variance vs. Mean (Log10(Raw Counts))")
```
...
plot_pca(sample_names, metadata, 'Raw Counts PCA')
```
...
plot_sample_distributions(cpm_tbl[-1], TRUE, "Sample Distribution (log10(CPM))")
plot_variance_vs_mean(cpm_tbl[-1], TRUE, "Variance vs Mean (log10(CPM))")
plot_pca(cpm_tbl[-1], metadata, 'CPM PCA')
...
plot_sample_distributions(norm_deseq[-1], TRUE, "Sample Distribution (log10(DESeq2 norm counts)")
plot_variance_vs_mean(norm_deseq[-1], TRUE, "Variance vs Mean (DESeq2)")
plot_pca(norm_deseq[-1], metadata, 'DESeq2 Normalized PCA')
